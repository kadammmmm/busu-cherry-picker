# Call Selector Widget — Developer Setup Guide

## Local Development & GitHub Setup
...
This guide walks you through setting up the Call Selector Widget project on your local machine, testing it, and publishing it to your own GitHub repository.
...
# Security (leave blank for local dev, required in production)
API_KEY=
ADMIN_KEY=
...
The project already exists. Clone it directly instead of creating files from scratch:
...
The key files are:
- `index.js` — Express server with Socket.IO, TTL cache, auth middleware, rate limiting
- `src/widget-call-selector.js` — Web Component widget source
- `src/build/bundle.js` — Compiled widget (generated by `npm run build`)
...
You should see:
```
[INFO] Call Selector Server started
[INFO] Port: 5000
...
The warnings are expected for local development.
